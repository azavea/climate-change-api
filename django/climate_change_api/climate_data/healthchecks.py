...
HISTORIC_YEARS = range(1950, 2006)
FUTURE_YEARS = range(2006, 2101)
...
def check_missing_data_sources():
    ...
    historical_scenarios = Scenario.objects.filter(name='historical').values_list('id', flat=True)
    future_scenarios = (ClimateDataSource.objects.distinct('scenario')
                                                 .exclude(scenario__in=historical_scenarios)
                                                 .values_list('scenario', flat=True))

    # convert years to strings because ints are not iterable
    historical_years_str = [str(x) for x in HISTORIC_YEARS]
    future_years_str = [str(x) for x in FUTURE_YEARS]

    expected_historic = len(distinct_models) * len(historical_scenarios) * len(historical_years_str)
    expected_future = len(distinct_models) * len(future_scenarios) * len(future_years_str)
    ...
    expected_set = set()
    [expected_set.add(tuple(x)) for x in itertools.product(historical_years_str,
                                                           distinct_models,
                                                           historical_scenarios)]

    [expected_set.add(tuple(x)) for x in itertools.product(future_years_str,
                                                           distinct_models,
                                                           future_scenarios)]
